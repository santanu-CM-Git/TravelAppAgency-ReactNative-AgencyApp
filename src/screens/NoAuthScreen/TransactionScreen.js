...
export default function TransactionScreen({  }) {
    ...
    const fetchTransactionHistory = useCallback(async (page = 1) => {
        try {
            ...
            // if (startDay && endDay) {
            //     params.start_date = startDay;
            //     params.end_date = endDay;
            // }
            ...
            const response = await axios.post(`${API_URL}/agent/transactions`, requestData, {
                params,
                headers: {
    ...
    const renderTransactionHistory = ({ item }) => {
        return (
            <View style={styles.itemContainer}>
                <Image source={{ uri: item?.customer?.profile_photo_url }} style={styles.avatar} />
                <View style={styles.detailsContainer}>
                    <Text style={styles.name}>{item?.customer?.first_name}</Text>
                    <Text style={styles.message}>{item?.booking?.package?.location?.name}</Text>
                ...
                <Text style={[styles.amount, item?.type == "debit" ? styles.negative : styles.positive]}>
