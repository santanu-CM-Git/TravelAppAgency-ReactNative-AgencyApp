...
export default function ChatListScreen({  }) {
    ...
    const fetchChatList = async (page = 1, isRefreshing = false) => {
        try {
            ...
            const response = await axios.post(`${API_URL}/agent/message-list`,{}, {
                params: {
                    page
                },
                headers: {
            ...
            if (response.data.response) {
                ...
                console.log(data, 'data from chat list screen');
    ...
    useEffect(() => {
        ...
        if (searchText.trim() === '') {
            ...
            const filtered = chatList.filter(item => 
                item.customer.full_name.toLowerCase().includes(searchText.toLowerCase())
            );
                ...
                renderItem={({ item }) => (
                    ...
                    >
                        ...
                        <View style={styles.chatDetails}>
                            <Text style={styles.chatName}>{item.customer.full_name}</Text>
                ...
                ListEmptyComponent={() => (
                    <View style={styles.emptyContainer}>
                        <Text style={styles.emptyText}>
                            {searchText ? 'No users found matching your search' : 'No messages yet'}
                        </Text>
                    </View>
                )}
            />
